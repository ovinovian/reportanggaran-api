...
class SistemLoginController extends Controller
{
    ...
    public function login_verifikasi(Request $req)
    {
        ...
        if (!$cekUser || empty($cekUser))
        {
            ...
            if (Hash::check($req->password, $cekUser->password)){
                $req->session()->put('username', $cekUser->username);
                $req->session()->put('uuid', $cekUser->uuid);
                return redirect()->route('dashboard')->with(['dataUser'=>$dataUser]);
            } else {
    ...
    public function log_out()
    {
        if(session()->has('uuid')){
            ...
            return redirect('/')->with('berhasil', 'Selamat tinggal, kamu berhasil keluar');
        }
    }
}